...
def generateData(frac, All, random_state):
    subsampling = All.sample(frac = frac, random_state = random_state)
...
def eval_ghat(theta, X, Y, T, delta, ineq):
    u = gHat1(theta, X, Y, T, delta, ineq)
    if u <= 0:
        return 0
    return 1
...
def getCandidateSolution(candidateData_X, candidateData_Y, candidateData_T, gHats, deltas, ineq):
    ...
    minimizer_options = {'disp': False, 'maxiter': 2000}
...
def run_experiments(worker_id, nWorkers, ms, numM, numTrials, mTest, All, ineq):
    ...
    random_state = (experiment_number + 1) * 99
    (testX, testY, testT) = generateData(mTest, All, random_state)
    for trial in range(numTrials):
        for (mIndex, m) in enumerate(ms):
            try:
                # Generate the training data, D
                base_seed = (experiment_number * numTrials) + 1
                random_state = base_seed + trial  # done to obtain common random numbers for all values of m
                (trainX, trainY, trainT) = generateData(m, All, random_state)
                ...
                if theta is not None:
                    ...
                    upper_bound = gHat1(theta, testX, testY, testT, deltas[0], ineq)
                ...
                if passedSafetyTest:
                    ...
                    upper_bound = gHat1(result, testX, testY, testT, deltas[0], ineq)
...
if __name__ == "__main__":
    ...
    ms = np.logspace(-3, -0.05, num=12)
    numM = len(ms)
    numTrials = 10  # 10 * 4 = 40 samples per fraction
    ...
    _ = ray.get(
        [run_experiments.remote(worker_id, nWorkers, ms, numM, numTrials, mTest, All, Inequality.T_TEST) for worker_id in
          range(1, nWorkers + 1)])
