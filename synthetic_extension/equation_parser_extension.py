...
def construct_expr_tree(rev_polish_notation, delta):
    """
    ...
    t = stack.pop()
    configure_delta(t, delta)
    return t


def configure_delta(t_node, delta):
...
def change_deltas(t_node, hash_map):
    ...
    for k, v in hash_map.items():
        if len(v) > 1:
            avg_v = sum(v)/len(v)
            change_delta_value(t_node, k, avg_v)
...
##########################
def eval_expr_tree_conf_interval(t_node, Y, predicted_Y, T, inequality, predict_bound, safety_size):
    """
    ...
    """
    if t_node is not None:
        l_x, u_x = eval_expr_tree_conf_interval(t_node.left, Y, predicted_Y, T, inequality, predict_bound, safety_size)
        l_y, u_y = eval_expr_tree_conf_interval(t_node.right, Y, predicted_Y, T, inequality, predict_bound, safety_size)
        if l_x is None and u_x is None:
            if isFunc(t_node.value):
                return eval_func_bound(t_node.value, Y, predicted_Y, T, t_node.delta, inequality, predict_bound, safety_size)
            # number value
...
def eval_func_bound(element, Y, predicted_Y, T, delta, inequality, predict_bound, safety_size):
    ...
    if inequality == Inequality.T_TEST:
        ...
        return eval_hoeffding(estimate, num_of_elements, 0, 1, delta)
    return None, None
...
def eval_hoeffding(estimate, num_of_elements, lower_X, upper_X, delta):
    constant_term = math.log(1/delta) / (2 * num_of_elements)
    int_size = (upper_X - lower_X) * constant_term
    return estimate - int_size, estimate + int_size


def predict_hoeffding(estimate, num_of_elements, lower_X, upper_X, delta):
    constant_term = math.log(1/delta) / (2 * num_of_elements)
    int_size = 2 * (upper_X - lower_X) * constant_term
    return estimate - int_size, estimate + int_size


def predict_hoeffding_modified(estimate, num_of_elements, safety_size, delta):
    constant_term1 = math.log(1/delta) / (2 * num_of_elements)
    constant_term2 = math.log(1 / delta) / (2 * safety_size)
