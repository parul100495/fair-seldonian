...
##########################
def eval_expr_tree_conf_interval_base(t_node, Y, predicted_Y, T, delta, inequality, predict_bound, safety_size, modified_h):
    """
    ...
    if t_node is not None:
        if t_node.right is not None and t_node.right.value is not None:
            ...
            child_delta = delta
        l_x, u_x = eval_expr_tree_conf_interval_base(t_node.left, Y, predicted_Y, T, child_delta, inequality, predict_bound, safety_size, modified_h)
        l_y, u_y = eval_expr_tree_conf_interval_base(t_node.right, Y, predicted_Y, T, child_delta, inequality, predict_bound, safety_size, modified_h)
        if l_x is None and u_x is None:
            if isFunc(t_node.value):
                return eval_func_bound(t_node.value, Y, predicted_Y, T, delta, inequality, predict_bound, safety_size, modified_h)
            # number value
